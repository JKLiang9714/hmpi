 ...
 */
static ompi_crcp_bkmrk_pml_peer_ref_t* find_peer(ompi_process_name_t proc);
                                  ...
                                  ompi_crcp_base_pml_state_t* pml_state)
{
     ...
     *****************************/
        ...
        for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
            ...
            cur_peer_ref->proc_name.jobid = OMPI_PROC_MY_NAME->jobid;
        }
...
static int ft_event_check_bookmarks(void)
{
    ...
    for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
        ...
        if( OMPI_PROC_MY_NAME->vpid < peer_ref->proc_name.vpid ) {
            /********************
             * Check P_n --> P_m
...
static void display_indv_timer_core(int idx, int proc, int msgs, bool direct) {
    ...
    if( 0 != OMPI_PROC_MY_NAME->vpid && timing_enabled < 3 ) {
        return;
    }
...
static void traffic_message_dump_peer(ompi_crcp_bkmrk_pml_peer_ref_t *peer_ref, char * msg, bool root_only)
{
    if( root_only && ompi_process_info.my_name.vpid != 0 ) {
        return;
    } else {
        sleep(ompi_process_info.my_name.vpid * 2);
    }
