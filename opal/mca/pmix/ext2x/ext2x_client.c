...
static void errreg_cbfunc (pmix_status_t status,
                           ...
                           void *cbdata)
{
    opal_ext2x_event_t *event = (opal_ext2x_event_t*)cbdata;
...
int ext2x_client_init(opal_list_t *ilist)
{
...
int ext2x_client_finalize(void)
{
    pmix_status_t rc;
...
int ext2x_abort(int flag, const char *msg,
                 opal_list_t *procs)
{
    pmix_status_t rc;
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            if (NULL == (nsptr = ext2x_convert_jobid(ptr->name.jobid))) {
...
int ext2x_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    pmix_value_t kv;
    pmix_status_t rc;
    pmix_proc_t p;
...
int ext2x_fence(opal_list_t *procs, int collect_data)
{
    pmix_status_t rc;
    opal_namelist_t *ptr;
    char *nsptr;
...
int ext2x_fencenb(opal_list_t *procs, int collect_data,
...
int ext2x_put(opal_pmix_scope_t opal_scope,
               opal_value_t *val)
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = ext2x_convert_opalscope(opal_scope);
    pmix_status_t rc;
...
int ext2x_get(const opal_process_name_t *proc, const char *key,
...
int ext2x_getnb(const opal_process_name_t *proc, const char *key,
                 opal_list_t *info,
...
int ext2x_publish(opal_list_t *info)
{
    pmix_info_t *pinfo;
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    return ext2x_convert_rc(ret);
}

int ext2x_publishnb(opal_list_t *info,
                     opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    opal_value_t *iptr;
...
int ext2x_lookup(opal_list_t *data, opal_list_t *info)
{
    opal_pmix_pdata_t *d;
    pmix_pdata_t *pdata;
    pmix_info_t *pinfo = NULL;
                      ...
                      void *cbdata)
{
    ...
    int rc;
    size_t n;
    opal_ext2x_jobid_trkr_t *job, *jptr;
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            d->proc.vpid = ext2x_convert_rank(data[n].proc.rank);
            d->value.key = strdup(data[n].key);
            rc = ext2x_value_unload(&d->value, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int ext2x_lookupnb(char **keys, opal_list_t *info,
...
int ext2x_unpublish(char **keys, opal_list_t *info)
{
    pmix_status_t ret;
    size_t ninfo, n;
    pmix_info_t *pinfo;
...
int ext2x_unpublishnb(char **keys, opal_list_t *info,
                       opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    ext2x_opcaddy_t *op;
...
int ext2x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    pmix_status_t rc;
...
int ext2x_spawnnb(opal_list_t *job_info, opal_list_t *apps,
...
int ext2x_connect(opal_list_t *procs)
{
    pmix_proc_t *p;
    size_t nprocs;
    opal_namelist_t *ptr;
...
int ext2x_connectnb(opal_list_t *procs,
...
int ext2x_disconnect(opal_list_t *procs)
{
    pmix_proc_t *p;
    size_t nprocs;
    opal_namelist_t *ptr;
...
int ext2x_disconnectnb(opal_list_t *procs,
                        opal_pmix_op_cbfunc_t cbfunc,
                        void *cbdata)
{
    ext2x_opcaddy_t *op;
...
int ext2x_resolve_peers(const char *nodename,
                         opal_jobid_t jobid,
                         opal_list_t *procs)
{
    pmix_status_t ret;
    ...
    if (OPAL_JOBID_WILDCARD != jobid) {
        if (NULL == (nspace = ext2x_convert_jobid(jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
    } else {
    ...
    if (NULL != array && 0 < nprocs) {
        ...
        for (n=0; n < nprocs; n++) {
            ...
            nm->name.vpid = ext2x_convert_rank(array[n].rank);
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    }
...
int ext2x_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    ...
    if (NULL == (nsptr = ext2x_convert_jobid(jobid))) {
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return OPAL_ERR_NOT_FOUND;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int ext2x_allocate(opal_pmix_alloc_directive_t directive,
                    opal_list_t *info,
...
int ext2x_job_control(opal_list_t *targets,
