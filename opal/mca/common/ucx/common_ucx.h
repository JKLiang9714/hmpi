                                            ...
                                            enum opal_common_ucx_req_type type)
{
    switch (type) {
        ...
        case OPAL_COMMON_UCX_REQUEST_TYPE_UCG:
            return ucg_request_check_status(request);
#endif
        default:
            break;
    }
...
static inline int opal_common_ucx_ep_flush(ucp_ep_h ep, ucp_worker_h worker)
{
    ...
    return opal_common_ucx_wait_request(request, worker, OPAL_COMMON_UCX_REQUEST_TYPE_UCP, "ucp_ep_flush_nb");
#else
    ucs_status_t status;
...
static inline int opal_common_ucx_worker_flush(ucp_worker_h worker)
{
    ...
    return opal_common_ucx_wait_request(request, worker, OPAL_COMMON_UCX_REQUEST_TYPE_UCP, "ucp_worker_flush_nb");
#else
    ucs_status_t status;
                                               ...
                                               ucp_worker_h worker)
{
    ...
    return opal_common_ucx_wait_request(request, worker, OPAL_COMMON_UCX_REQUEST_TYPE_UCP, "ucp_atomic_fetch_nb");
}

static inline int opal_common_ucx_atomic_cswap(ucp_ep_h ep, uint64_t compare,
                                               uint64_t value, void *result, size_t op_size,
                                               uint64_t remote_addr, ucp_rkey_h rkey,
                                               ucp_worker_h worker)
{
