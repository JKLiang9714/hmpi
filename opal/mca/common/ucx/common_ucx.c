...
OPAL_DECLSPEC int opal_common_ucx_mca_pmix_fence(ucp_worker_h worker)
{
    ...
    if (OPAL_SUCCESS != (ret = opal_pmix.fence_nb(NULL, 0, opal_common_ucx_mca_fence_complete_cb, (void*)&fenced))) {
        return ret;
    }
        ...
        if (dreq != NULL) {
            if (UCS_PTR_IS_ERR(dreq)) {
                ...
                if (num_reqs >= max_disconnect) {
                    opal_common_ucx_wait_all_requests(dreqs, num_reqs, worker, OPAL_COMMON_UCX_REQUEST_TYPE_UCP);
                    num_reqs = 0;
                }
            }
        }
