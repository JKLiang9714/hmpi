...
MSPACES                  default: 0 (false)
  ...
  This is only supported if DL_HAVE_MMAP is true.
...
DL_DEBUG                    default: NOT defined
  The DL_DEBUG setting is mainly intended for people trying to modify
  this code or diagnose problems when porting to new platforms.
  However, it may also be able to better isolate user errors than just
  using runtime checks.  The assertions in the check routines spell
  out in more detail the assumptions and invariants underlying the
  algorithms.  The checking is fairly extensive, and will slow down
  execution noticeably. Calling malloc_stats or mallinfo with DL_DEBUG
...
DL_HAVE_MREMAP               default: 1 on linux, else 0
...
MAX_RELEASE_CHECK_RATE   default: 255 unless not DL_HAVE_MMAP
...
#ifdef linux
#define DL_HAVE_MREMAP 1
#else   /* linux */
#define DL_HAVE_MREMAP 0
#endif  /* linux */
#endif  /* DL_HAVE_MREMAP */
...
/*
   If HAVE_MORECORE or DL_HAVE_MMAP are false, we just define calls and
...
#if DL_HAVE_MMAP && DL_HAVE_MREMAP
#define CALL_MREMAP(addr, osz, nsz, mv) mremap((addr), (osz), (nsz), (mv))
#else  /* DL_HAVE_MMAP && DL_HAVE_MREMAP */
#define CALL_MREMAP(addr, osz, nsz, mv) ((void)(addr),(void)(osz), \
                                         (void)(nsz), (void)(mv),MFAIL)
#endif /* DL_HAVE_MMAP && DL_HAVE_MREMAP */
...
static FORCEINLINE void pthread_release_lock (MLOCK_T *sl) {
  int ret;
  dl_assert(CURRENT_THREAD==sl->threadid);
  if (!--sl->c) {
    sl->threadid=0;
...
/* Modified by sasha: also check that address is within memheap */
#define ok_magic(M)      ((char *)(M) >= (char *)gm->least_addr && (M)->magic == mparams.magic)
...
static void do_check_any_chunk(mstate m, mchunkptr p) {
  dl_assert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dl_assert(ok_address(m, p));
}
...
static void do_check_top_chunk(mstate m, mchunkptr p) {
  ...
  dl_assert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dl_assert(ok_address(m, p));
  dl_assert(sz == m->topsize);
  dl_assert(sz > 0);
  dl_assert(sz == ((sp->base + sp->size) - (char*)p) - TOP_FOOT_SIZE);
  dl_assert(pinuse(p));
  dl_assert(!pinuse(chunk_plus_offset(p, sz)));
}
...
static void do_check_mmapped_chunk(mstate m, mchunkptr p) {
  ...
  dl_assert(use_mmap(m));
  dl_assert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dl_assert(ok_address(m, p));
  dl_assert(!is_small(sz));
  dl_assert((len & (mparams.page_size-SIZE_T_ONE)) == 0);
  dl_assert(chunk_plus_offset(p, sz)->head == FENCEPOST_HEAD);
  dl_assert(chunk_plus_offset(p, sz+SIZE_T_SIZE)->head == 0);
}
...
static void do_check_inuse_chunk(mstate m, mchunkptr p) {
  ...
  dl_assert(cinuse(p));
  dl_assert(next_pinuse(p));
  /* If not pinuse and not mmapped, previous chunk has OK offset */
  dl_assert(is_mmapped(p) || pinuse(p) || next_chunk(prev_chunk(p)) == p);
  if (is_mmapped(p))
...
static void do_check_malloced_chunk(mstate m, void* mem, size_t s) {
  if (mem != 0) {
    ...
    dl_assert(is_mmapped(p) || sz < (s + MIN_CHUNK_SIZE));
  }
}
...
static void do_check_tree(mstate m, tchunkptr t) {
  ...
  dl_assert((idx == NTREEBINS-1) || (tsize < minsize_for_tree_index((idx+1))));

  do { /* traverse through chain of same-sized nodes */
    do_check_any_chunk(m, ((mchunkptr)u));
    dl_assert(u->index == tindex);
    dl_assert(chunksize(u) == tsize);
    dl_assert(!cinuse(u));
    dl_assert(!next_pinuse(u));
    dl_assert(u->fd->bk == u);
    dl_assert(u->bk->fd == u);
    if (u->parent == 0) {
      ...
      if (u->child[0] != 0 && u->child[1] != 0) {
        dl_assert(chunksize(u->child[0]) < chunksize(u->child[1]));
      }
    }
    u = u->fd;
  } while (u != t);
...
static void do_check_smallbin(mstate m, bindex_t i) {
  ...
  if (!empty) {
    for (; p != b; p = p->bk) {
      ...
      dl_assert(small_index(size) == i);
      dl_assert(p->bk == b || chunksize(p->bk) == chunksize(p));
  ...
  if (is_initialized(m)) {
    ...
    while (s != 0) {
      ...
      while (segment_holds(s, q) &&
        ...
        sum += chunksize(q);
        if (cinuse(q)) {
          dl_assert(!bin_find(m, q));
          do_check_inuse_chunk(m, q);
        }
        else {
          dl_assert(q == m->dv || bin_find(m, q));
          dl_assert(lastq == 0 || cinuse(lastq)); /* Not 2 consecutive free */
...
static void do_check_malloc_state(mstate m) {
  ...
  if (m->dvsize != 0) { /* check dv chunk */
    ...
    dl_assert(m->dvsize == chunksize(m->dv));
    dl_assert(m->dvsize >= MIN_CHUNK_SIZE);
    dl_assert(bin_find(m, m->dv) == 0);
  }

  if (m->top != 0) {   /* check top chunk */
    do_check_top_chunk(m, m->top);
    /*dl_assert(m->topsize == chunksize(m->top)); redundant */
    dl_assert(m->topsize > 0);
    dl_assert(bin_find(m, m->top) == 0);
  }

  total = traverse_and_check(m);
  dl_assert(total <= m->footprint);
  dl_assert(m->footprint <= m->max_footprint);
}
#endif /* DL_DEBUG */
...
#define insert_small_chunk(M, P, S) {\
  bindex_t IDX  = small_index(S);\
  mchunkptr B = smallbin_at(M, IDX);\
  mchunkptr F = B;\
...
#define unlink_small_chunk(M, P, S) {\
  ...
  else if (RTCHECK((F == smallbin_at(M,IDX) || ok_address(M, F)) &&\
                   (B == smallbin_at(M,IDX) || ok_address(M, B)))) {\
    F->bk = B;\
    B->fd = F;\
  }\
  else {\
...
#define unlink_first_small_chunk(M, B, P, IDX) {\
  mchunkptr F = P->fd;\
  dl_assert(P != B);\
  dl_assert(P != F);\
  dl_assert(chunksize(P) == small_index2size(IDX));\
...
#define insert_large_chunk(M, X, S) {\
  ...
  if (!treemap_is_marked(M, IDX)) {\
    ...
    *H = X;\
    ...
    tchunkptr T = *H;\
    size_t K = S << leftshift_for_tree_index(IDX);\
    for (;;) {\
      if (chunksize(T) != S) {\
...
static void* sys_alloc(mstate m, size_t nb) {
  ...
  /*
    ...
    2. A call to MMAP new space (disabled if not DL_HAVE_MMAP).
       Note that under the default settings, if MORECORE is unable to
       fulfill a request, and DL_HAVE_MMAP is true, then mmap is
  ...
  */
  ...
  if (DL_HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
...
static size_t release_unused_segments(mstate m) {
  ...
  /* Reset check counter */
  m->release_checks = ((nsegs > (int)MAX_RELEASE_CHECK_RATE)?
                       (size_t)nsegs : MAX_RELEASE_CHECK_RATE);
  return released;
}
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    if (mem != 0) {
      ...
      dl_assert((((size_t)(chunk2mem(p))) % alignment) == 0);
...
//printf("checking state m=%p gm=%p least_addr=%p p=%p, head=%x size=%d pp=%x\n", m, gm, gm->least_addr, p, (unsigned)p->head, (int)chunksize(p),
 //       (unsigned)(((mchunkptr)((char*)(p) +(chunksize(p))))->prev_foot)
  //      );
    if (!ok_magic(m)) {
//printf("checking state - oops\n");
      USAGE_ERROR_ACTION(m, oldmem);
      return 0;
    }
//printf("checking state OK\n");
#endif /* FOOTERS */
//printf("to internal realloc m=%p gm=%p, mparams.magic=%x oldmem=%p bytes=%d\n", m, gm, (unsigned)mparams.magic, oldmem, (int)bytes);
