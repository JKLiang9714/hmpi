 ...
 * Object stuff
 **********/
struct  orte_sstore_stage_app_snapshot_info_t {
    /** List super object */
...
typedef struct orte_sstore_stage_app_snapshot_info_t orte_sstore_stage_app_snapshot_info_t;
ORTE_DECLSPEC OBJ_CLASS_DECLARATION(orte_sstore_stage_app_snapshot_info_t);

void orte_sstore_stage_app_snapshot_info_construct(orte_sstore_stage_app_snapshot_info_t *info);
void orte_sstore_stage_app_snapshot_info_destruct( orte_sstore_stage_app_snapshot_info_t *info);

OBJ_CLASS_INSTANCE(orte_sstore_stage_app_snapshot_info_t,
                   opal_list_item_t,
 ...
 **********/
static orte_sstore_stage_app_snapshot_info_t *create_new_handle_info(orte_sstore_base_handle_t handle);
static orte_sstore_stage_app_snapshot_info_t *find_handle_info(orte_sstore_base_handle_t handle);

static int init_local_snapshot_directory(orte_sstore_stage_app_snapshot_info_t *handle_info);
...
static int metadata_open(orte_sstore_stage_app_snapshot_info_t * handle_info);
static int metadata_close(orte_sstore_stage_app_snapshot_info_t * handle_info);
static int metadata_write_str(orte_sstore_stage_app_snapshot_info_t * handle_info, char * key, char *value);
static int metadata_write_int(orte_sstore_stage_app_snapshot_info_t * handle_info, char *key, int value);
static int metadata_write_timestamp(orte_sstore_stage_app_snapshot_info_t * handle_info);
...
{
    opal_output(0, "sstore:stage:(app): request_checkpoint_handle() Not implemented!");
...
int orte_sstore_stage_app_register(orte_sstore_base_handle_t handle)
{
    ...
    orte_sstore_stage_app_snapshot_info_t *handle_info = NULL;

    OPAL_OUTPUT_VERBOSE((10, mca_sstore_stage_component.super.output_handle,
                         "sstore:stage:(app): register(%d)", (int)handle));
...
int orte_sstore_stage_app_get_attr(orte_sstore_base_handle_t handle, orte_sstore_base_key_t key, char **value)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, mca_sstore_stage_component.super.output_handle,
                         "sstore:stage:(app): get_attr(%d)", key));
...
int orte_sstore_stage_app_set_attr(orte_sstore_base_handle_t handle, orte_sstore_base_key_t key, char *value)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, mca_sstore_stage_component.super.output_handle,
                         "sstore:stage:(app): set_attr(%d = %s)", key, value));
...
static orte_sstore_stage_app_snapshot_info_t *create_new_handle_info(orte_sstore_base_handle_t handle)
{
    ...
    handle_info = OBJ_NEW(orte_sstore_stage_app_snapshot_info_t);
...
static orte_sstore_stage_app_snapshot_info_t *find_handle_info(orte_sstore_base_handle_t handle)
{
    ...
    for(item  = opal_list_get_first(active_handles);
        ...
        handle_info = (orte_sstore_stage_app_snapshot_info_t*)item;
...
static int init_local_snapshot_directory(orte_sstore_stage_app_snapshot_info_t *handle_info)
{
    ...
    if(OPAL_SUCCESS != (ret = opal_os_dirpath_create(handle_info->local_location, my_mode)) ) {
        opal_show_help("help-orte-sstore-stage.txt", "fail_path_create", true,
                       ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
